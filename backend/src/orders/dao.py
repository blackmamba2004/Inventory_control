...
class OrderDAO(BaseDAO):
    ...
    async def create(cls, session: AsyncSession, data: list[ProductItem]):
        async with session.begin():
            ...
            total_price, order_items = await cls.add_order_items(session, order.id, data)
    ...
    @classmethod
    async def add_order_items(cls, session: AsyncSession,
                               order_id: int, data: list[ProductItem]):
        ...
        for item in data:
            ...
            await OrderItemDAO.create_order_item(session, order_id,
                                         product.id, requested_count)

            order_items_responses.append(OrderItemDAO
                                         .build_order_item_json(product, requested_count))
    ...
    def build_full_order_json(cls, order: Order):
        ...
        for order_item in order.order_items:
            ...
            order_items_responses.append(OrderItemDAO
                                         .build_order_item_json(order_item.product, order_item.count))
...
class OrderItemDAO(BaseDAO):
    ...
    @classmethod
    async def create_order_item(cls, session: AsyncSession, order_id: int,
                                 product_id: int, requested_count: int):
        order_item = OrderItem(order_id=order_id, product_id=product_id, count=requested_count)
