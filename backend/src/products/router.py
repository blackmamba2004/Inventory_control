...
async def create_product_many(product: list[CreateProduct], 
                              session: AsyncSession = Depends(get_db)):
...
async def update_product(product_id: int, 
                         ...
                         session: AsyncSession = Depends(get_db)):
    return await ProductDAO.update(session, product_id, 
                                   updated_data, partial=False)


@router.patch('/{product_id}', response_model=FullProductResponse, status_code=200)
async def partial_update_product(product_id: int, 
                                 updated_data: UpdateProduct, 
                                 session: AsyncSession = Depends(get_db)):
    return await ProductDAO.update(session, product_id, 
                                   updated_data, partial=True)
